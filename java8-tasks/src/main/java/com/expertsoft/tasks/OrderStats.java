...
class OrderStats {
    ...
    static Map<Integer, List<Order>> orderSizes(final Stream<Order> orders) {
        return orders
                .collect(Collectors.groupingBy(order -> order.getOrderItems().stream().mapToInt(OrderItem::getQuantity).sum()));
    }
    ...
    static Map<String, Long> cardsCountForCustomer(final Stream<Customer> customers) {
        return customers
                .collect(Collectors.toMap(Customer::getEmail,
    ...
    static Optional<String> mostPopularCountry(final Stream<Customer> customers) {
        return customers
                .collect(Collectors.groupingBy(customer -> customer.getAddress().getCountry())) //String, LIst<Customer>
        ...
        return customers
                ...
                .flatMap(orderItem ->
                        IntStream.range(0, orderItem.getQuantity())
                                .mapToObj(i -> orderItem.getProduct().getPrice()))
                .collect(collector);
    }
}
